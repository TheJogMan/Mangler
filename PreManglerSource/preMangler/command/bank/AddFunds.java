...
public class AddFunds extends AbstractCommand
{
	public AddFunds(CommandParent parent)
	{
		super(parent, "AddFunds", "Adds funds to a player's current cash balance.");
	...
	public void executeCommand(MultiListInterpretationResult result, AbstractExecutor executor)
	{
		if (hasAuthority(executor))
		{
			...
			executor.respond("$" + amount + " has been added to " + person.getColoredName() + "'s balance, their balance is now $" + balance + ".");
		}
		else
